...
md"""
$br
Number of simulated datasets: $(@bind n_datasets NumberField(100:100:10_000, default=400)) 
...
@bind redraw_for_plot2 Button("Reassign training/test sets") 

# ╔═╡ 484fd7a0-76d7-464d-a414-0ba48d7304b8
md"""
Order of polynominal to use for cross validation: $(@bind order_cv NumberField(1:3, default=1)) 
"""
...
begin
	redraw_for_plot 
	x_obs, y_obs, design_matrix, y_true, σ_true, idx_outliers = generate_data(θ_true[1:(order_true+1)], n_obs, σ_obs,n_outliers=n_outliers)
...
begin
	redraw_for_plot2
	x_obs_shuffle, y_obs_shuffle = shuffleobs((x_obs, y_obs))
	cv_data, test_data = splitobs((x_obs_shuffle, y_obs_shuffle); at=0.75)
	n_obs_cv = length(cv_data[1])
	n_obs_test = length(test_data[1])
	plt2 = plot(xlabel=L"x", ylabel=L"y_{obs}", legend=:topleft)
	loss_per_dof = round(loss/(n_obs), sigdigits=3)
	A_all = mapreduce(i->x_obs.^i,hcat,0:order_cv) # design matrix
	
	order_fit = 3
	A_cv = mapreduce(i->cv_data[1].^i,hcat,0:order_cv) # design matrix
	θ_cv_fit = calc_mle_linear_model(A_cv,cv_data[2], diagm(fill(σ_obs,n_obs_cv)))
	y_cv_pred = predict_linear_model(A_cv,θ_cv_fit)
	loss_cv = round(sum(((cv_data[2].-y_cv_pred)./σ_obs).^2)/(n_obs_cv-order_fit-1),sigdigits=3)
	y_cv_pred = predict_linear_model(A_all,θ_cv_fit)
	
	A_test = mapreduce(i->test_data[1].^i,hcat,0:order_cv) # design matrix
	θ_test_fit = calc_mle_linear_model(A_test,test_data[2], diagm(fill(σ_obs,n_obs_test)))
	y_test_pred = predict_linear_model(A_test,θ_cv_fit)
	loss_test = round(sum(((test_data[2].-y_test_pred)./σ_obs).^2)/(n_obs_test-order_fit-1),sigdigits=3)
	#y_test_pred = predict_linear_model(A_all,θ_test_fit)
	#plot!(plt2, x_obs,y_test_pred, lineweight=4, lc=:red, label="Best-fit to test set: χ²/dof=$(loss_test)")
	
	plot!(plt2, x_obs,y_true, lineweight=4, lc=:black, label="True χ²/dof=$(loss_per_dof)")
	plot!(plt2, x_obs,y_cv_pred, lineweight=4, lc=:blue, label="Best-fit to training set")
	scatter!(plt2,cv_data[1], cv_data[2], yerr=fill(σ_obs,n_obs_cv), mc=:blue, label="Training χ²/dof=$(loss_cv)")
	scatter!(plt2,test_data[1], test_data[2], yerr=fill(σ_obs,n_obs_test), mc=:red, label="Test χ²/dof=$(loss_test)")
	plt2	
end
...
begin
    num_cv_draws = 400
	θ_cv_dist = stack(x->view(θ_true,1:(order_cv+1)), 1:num_cv_draws)
	loss_cv_dist = zeros(num_cv_draws)
	loss_test_dist = zeros(num_cv_draws)
	for i in 1:num_cv_draws
		local x_obs_shuffle, y_obs_shuffle = shuffleobs((x_obs, y_obs))
		local cv_data, test_data = splitobs((x_obs_shuffle, y_obs_shuffle); at=0.75)
		local n_obs_cv = length(cv_data[1])
		local n_obs_test = length(test_data[1])
		local order_fit = order_cv
		local A_cv = mapreduce(i->cv_data[1].^i,hcat,0:order_cv) # design matrix
		θ_cv_dist[:,i] .= calc_mle_linear_model(A_cv,cv_data[2], diagm(fill(σ_obs,n_obs_cv)))
		local y_cv_pred = predict_linear_model(A_cv,θ_cv_fit)
		loss_cv_dist[i] = round(sum(((cv_data[2].-y_cv_pred)./σ_obs).^2)/(n_obs_cv-order_fit-1),sigdigits=3)		
		local A_test = mapreduce(i->test_data[1].^i,hcat,0:order_cv) # design matrix
		local y_test_pred = predict_linear_model(A_test,θ_cv_fit)
		loss_test_dist[i] = round(sum(((test_data[2].-y_test_pred)./σ_obs).^2)/(n_obs_test-order_fit-1),sigdigits=3)
	end
...
begin
	loss_all_dist = vcat(loss_cv_dist,loss_test_dist)
	local bins = range(minimum(loss_all_dist), stop=maximum(loss_all_dist), length=20)
...
let
	plt1 = plot_parameter_histograms(θ_cv_dist, θ_true,title="CV Distributions")
	#plt2 = plot_parameter_histograms(results_linear_model, θ_true, title="Posterior Sample")
...
manifest_format = "2.0"
project_hash = "fc630e558cfdc69ae9a3993b52040c01986e8bf0"
