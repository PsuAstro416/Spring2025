...
# ╔═╡ f18229a3-05e8-4270-9328-8e14385b100a
# hideall
begin
...
# ╔═╡ 1c6609b2-d294-4920-9bd4-dbcadd13facd
# hideall
md"""
+++
title = $topic
...
# ╔═╡ 8bf8137b-fb30-4fb5-87bb-16628f0a5f60
md"""
Recall the model for general least squares linear regression
$y = A \theta + \epsilon,$
where:
- the vector of measurement values is $y_{\mathrm{obs}}$, 
- the **[design matrix](https://en.wikipedia.org/wiki/Design_matrix)** is $A$,
...
# ╔═╡ c728f0e2-b5fb-49bd-a105-baaccd5b90df
md"""
...
# ╔═╡ d4e3046d-8ee1-435e-abb4-50772eef5222
md"""
## L2 Regularization
...
# ╔═╡ 0401e047-795d-4f22-b6d7-e05ac1a450ea
md"""
## L1 Regularization

$\mathrm{loss}_{L1,\lambda}(\theta) = -\frac{1}{2}\left(y-A \theta\right)^T \Sigma^{-1} \left(y-A \theta\right)
...
I.e., we can find fits that has less non-zero parameters!
...
Unfortunately, there is no analtyic solution for $\hat{\theta}_{L1}$.  
Instead, there are itterative algorithms for estimating $\hat{\theta}_{L1}$.
"""

# ╔═╡ afafec68-525f-426a-ac16-c0cd5a7eec8b
md"""
## Bayesian Interpretation 
In a Bayesian setting, we would want to work with the posterior distribution for $\theta$.  
If we assume a uniform prior for $\theta$, then the posterior distribution becomes 
$\theta \sim \mathrm{Normal}(\hat{\theta}_{MLE}, \Sigma_{\theta_{MLE}}), $
where $\Sigma_{\theta_{MLE}} = (A^{T} \Sigma^{-1}  A)^{-1}$.


"""

# ╔═╡ e0b6fa90-e614-49f2-b8fd-8ec4a12c5ec2
md"""
Since 
...
$\mathrm{loss}_{MAP}(\theta) = -\frac{1}{2}\left(y-A \theta\right)^T \Sigma^{-1} \left(y-A \theta\right)
-\frac{1}{2}\left(\theta-\theta_0\right)^T \Sigma_{pr}^{-1} \left(\theta-\theta_0\right) + \mathrm{const}.$

If we set $\theta_0 = 0$ and $\Sigma_{pr}^{-1} = \lambda I$, then this reduces to 

$\mathrm{loss}_{MAP}(\theta) = -\frac{1}{2}\left(y-A \theta\right)^T \Sigma^{-1} \left(y-A \theta\right)
-\frac{\lambda}{2}\left|\theta\right|^2  + \mathrm{const}$
...
# ╔═╡ 2115e110-af8d-4ed3-ad9e-f89138e066ed
md"""
...
# ╔═╡ eeb3768d-9f1d-4396-b907-e4ecf0cb7464
md"""
# Example Application to Fitting a Spectrum
"""

# ╔═╡ 9ed3e8ee-b688-4542-8caf-77ae381df6d2
md"""
## Function to be approximated
"""

# ╔═╡ cba8a943-4158-4c87-9e06-6166468d10c9
...
md"""
Regularly spaced data: $(@bind x_obs_regularly_spaced CheckBox(;default=false))
"""
...
filter_small_coef_from_histo = true;
...
# ╔═╡ a82083ea-c6e2-408d-9e54-018720685cc3
md"""
## Perform Fitting
"""
...
"Solve for best-fit linear coefficients from design matrix A and data y"
...
# ╔═╡ a05da4f0-d1c5-4d11-8b06-0d731f3672a5
 "Solve for best-fit linear coefficients from design matrix A and data y, with L2 regularization penality on coefficients of magnitude 10^logλ"
 function fit_L2(A::AbstractArray{T,2}, y::AbstractVector; logλ::Real = 1) where {T<:Number}
	ATA = A' * A
	ATA[diagind(ATA)] .+= 10^logλ
	ATy = A' * y
	ATA \ ATy 
end 

# ╔═╡ 1088b302-7f94-4ffd-a4e3-89f3a6db2617
"""
    ...
    Args:
        A (Matrix): The design matrix for the linear problem, shape (N, M).
        y (Vector): The data vector, length N.
        logλ (Real, optional): The log of the regularization strength parameter, λ. Defaults to 5.0.
        maxiter (Int, optional): Maximum number of iterations. Defaults to 9999.
        eps (Real, optional): Precision of the algorithm. Defaults to 1e-15.
        tol (Real, optional): Iteration stop tolerance. Defaults to 1e-8.

    Returns:
        Vector: The vector of coefficients w that minimizes the L1 norm
            for the linear problem.

	Credits:  Code from https://github.com/Astroinformatics/Regularization translated from Python to Julia by Eric Ford (February 2025)
"""
function fit_L1(A::AbstractArray{T,2}, y::AbstractVector; logλ::Real=5.0, maxiter::Int=9999, ϵ::Real=1e-15, tol::Real=1e-8) where { T<:Number }
    ATA = A' * A
    KInv = copy(ATA)   
	ATy = A' * y
    w = ones(size(ATA, 1))
	absw = ones(size(ATA, 1))
    w_new = ones(size(ATA, 1))
	λ = 10.0^logλ
    for n in 1:maxiter
        absw[absw .< λ * ϵ] .= λ * ϵ
        KInv .= ATA
        KInv[diagind(KInv)] .+= λ ./ absw
        w_new .= KInv \ ATy
        sum_sq_Δw = sum(abs2.(w .- w_new))
        w .= w_new
        absw .= abs.(w)
        if sum_sq_Δw < tol
            break
...
# ╔═╡ eae4ebe7-0ab1-4528-8787-49c6e220f66d
"""A design matrix constructed out of the Legendre polynomial basis.

Inputs:
- x (Vector): The independent coordinate vector, length `N`.
- ncol (Int): The number of columns `M` in the design matrix.  Note that this corresponds to a polynomial fit with highest order equal to `M - 1`.

Outputs:
- The design matrix for the linear problem, shape `(N, M)`.
    """    
function poly_design_matrix(x::AbstractVector, ncol::Integer)
...
"""`plot_diamond!(plt, xc, yc; scale=1, n=100, kwargs...)`"""
function plot_diamond!(plt, xc, yc; scale::Real=1, n::Integer = 100, kwargs...)
	x = scale/2 .* vcat( range(-1,stop=1,length=n), ones(n), reverse(range(-1,stop=1,length=n)), -ones(n))
	y = scale/2 .* vcat( ones(n), reverse(range(-1,stop=1,length=n)), -ones(n), range(-1,stop=1,length=n) )
	(sθ, cθ) = sincosd(45)
	xx = xc .+ ( cθ * x .+ -sθ * y)
	yy = yc .+ ( sθ * x .+ cθ * y )
...
# ╔═╡ 8059f6f5-30b8-41a6-a320-c32fb6f6c77e
"""`plot_elipse!(plt, x, y; rotation=0, scale_x=1, scale_y=1,  n=100, kwargs...)`"""
function plot_elipse!(plt, x, y; rotation::Real=0, scale_x::Real=1, scale_y::Real=1,  n::Integer = 100, kwargs...)
	t = range(0, stop=2,length=n)
	ct = scale_x .* cospi.(t)
	st = scale_y .* sinpi.(t)
	(sθ, cθ) = sincosd(rotation)
	xx = x .+ ( cθ * ct .+ -sθ * st)
	yy = y .+ ( sθ * ct .+ cθ * st )
	plot!(plt, xx,yy; label=:none, kwargs...)
	plt
...
manifest_format = "2.0"
project_hash = "7e8e9a447e2f9c585feda1c46d76c83faf1b34e1"
