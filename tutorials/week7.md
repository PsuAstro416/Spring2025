...
<pre class="code-output documenter-example-output" id="var-hash273398">BenchmarkTools.Trial: 2311 samples with 1 evaluation per sample.
 Range (min … max):  925.269 μs … 10.959 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     947.010 μs              ┊ GC (median):    0.00%
 Time  (mean ± σ):     2.113 ms ±  1.926 ms  ┊ GC (mean ± σ):  0.00% ± 0.00%

  █                            ▁▅                            ▃  
  █▅▄▄▅▃▄▄▄▃█▅▄▅▅▄▄▃▁▃█▅▄▅▄▄▅▆▅██▅▄▄▅▄▅▄▁▇▇▁▁▃▃▃▃▄▁▇▅▁▄▁▁▃▁▁▁█ █
  925 μs        Histogram: log(frequency) by time      6.96 ms &lt;
...
<pre class="code-output documenter-example-output" id="var-hash120226">BenchmarkTools.Trial: 283 samples with 1 evaluation per sample.
 Range (min … max):   9.393 ms … 27.550 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     18.454 ms              ┊ GC (median):    0.00%
 Time  (mean ± σ):   17.679 ms ±  3.716 ms  ┊ GC (mean ± σ):  0.00% ± 0.00%

  ▄                            █         ▄                     
  █▄▁▁▁▁▁▁▁▆▆▁▁▁▁▁▁▄▁▇▄▁▁▆▁▁▅▁▄█▅▁▅▄▁▁▅▁▁█▁▁▁▁▁▁▄▁▁▄▁▁▁▁▁▄▁▁▄ ▅
  9.39 ms      Histogram: log(frequency) by time      27.4 ms &lt;
...
<pre class="code-output documenter-example-output" id="var-hash433772">BenchmarkTools.Trial: 2566 samples with 1 evaluation per sample.
 Range (min … max):  925.299 μs … 15.625 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     946.635 μs              ┊ GC (median):    0.00%
 Time  (mean ± σ):     1.935 ms ±  1.633 ms  ┊ GC (mean ± σ):  0.00% ± 0.00%
...
<pre class="code-output documenter-example-output" id="var-hash164123">BenchmarkTools.Trial: 105 samples with 1 evaluation per sample.
 Range (min … max):  28.241 ms … 75.833 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     52.063 ms              ┊ GC (median):    0.00%
 Time  (mean ± σ):   47.608 ms ± 12.937 ms  ┊ GC (mean ± σ):  0.00% ± 0.00%

  █                                █   ▄                       
  █▁▄▃▁▁▁▄▃▃▄▁▃▄▄▁▄▃▄▁▄▁▄▄▃▃▃▁▁▄▄▄▃█▄▃▄█▁▁▁▁▃▁▁▁▁▃▁▁▁▁▁▁▁▃▄▁▄ ▃
  28.2 ms         Histogram: frequency by time        75.6 ms &lt;
...
<pre class="code-output documenter-example-output" id="var-hash168870">BenchmarkTools.Trial: 4648 samples with 1 evaluation per sample.
 Range (min … max):  925.770 μs …   7.955 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     946.549 μs               ┊ GC (median):    0.00%
 Time  (mean ± σ):     1.067 ms ± 611.411 μs  ┊ GC (mean ± σ):  0.00% ± 0.00%

  █▁                                                          ▂  
  ██▆▅▄▅▅▃▃▄▃▃▁▁▁▃▁▃▁▁▄▁▁▃▁▃▃▄▄▃▄▁▄▃▁▃▁▃▃▁▄▄▁▁▃▁▃▃▅▃▃▁▁▄▃▃▄▄▁▄█ █
  926 μs        Histogram: log(frequency) by time       3.96 ms &lt;
...
<pre class="code-output documenter-example-output" id="var-hash964623">BenchmarkTools.Trial: 1795 samples with 1 evaluation per sample.
 Range (min … max):  2.731 ms …  4.165 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     2.767 ms              ┊ GC (median):    0.00%
 Time  (mean ± σ):   2.783 ms ± 55.813 μs  ┊ GC (mean ± σ):  0.00% ± 0.00%

         ▂▅▅▃▄▅▆▂▂▂                    █▅▁                    
  ▁▁▂▃▄▆▇██████████▆▇▄▄▃▂▂▃▂▂▂▂▂▃▅▅▄▃▅████▆▃▃▂▂▂▂▂▂▁▂▂▂▁▁▁▁▁ ▃
  2.73 ms        Histogram: frequency by time        2.86 ms &lt;
